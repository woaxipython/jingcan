...
class DouYinSpyder():
    ...
    def testCookie(self, token, webid, msToken, url=""):
    ...
    def getNoteInfo(self, token, webid, msToken, url=""):
        ...
        if not notice:
            return {"status": "2", "message": "笔记不存在"}
        data = response.json()['aweme_detail']
        if not data:
            return {"status": "2", "message": "登录已过期"}
        result = get_note_info(data, url, modal_id)
        return {"status": "1", "message": result}

    def getNoteList(self, token, webid, msToken, url="", max_cursor="0"):
        self.headers['cookie'] = token
